 ...
 * OneDrive/SharePoint share links: restricting to Microsoft's own domains makes DNS
 * rebinding irrelevant for the default configuration, because an attacker who does not
 * control DNS for these domains cannot rebind them to an internal address. Callers may
 * override via `fetchRemoteFile`'s `allowedHosts` option.
 */
...
/**
 * Reduces a filename to its basename and drops path separators — this module is a
 * trust boundary: the name arrives from a foreign server's `Content-Disposition` (or,
 * via routes.ts, from a request header) and ends up in a multipart field, in the
 * bookkeeping, and in everything that logs it.
 *
 ...
 * consists only of them comes back `undefined` and the caller's existing fallback
 * chain (URL basename / ticket value / `upload.bin`) picks a real name.
 *
 ...
 *
 * Two layers of defense, checked at every hop:
 *  1. Host allowlist (`allowedHosts`, default: Microsoft file-sharing domains). This is
 *     the primary control — it also makes DNS rebinding moot for the default config,
 *     since passing the IP check and then having `fetchImpl`/undici re-resolve to a
 *     different (internal) address on the actual connection is only exploitable if the
 *     attacker controls DNS for an allowed domain.
 *  2. Resolved-address check (`isBlockedAddress`) — defense in depth, not the only lock.
 */

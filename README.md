...
| `OAUTH_DISCOVERY` | `true` | Read the issuer's own authorization-server metadata at startup (RFC 8414, falling back to OIDC discovery) and advertise what it actually says, instead of guessing endpoints from the WorkOS URL layout. A document whose `issuer` does not match is discarded. If the issuer is unreachable the derived defaults are used and a warning is logged — discovery never blocks startup. Set `false` to skip the fetch |

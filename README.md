...
| `OAUTH_SCOPES_SUPPORTED` | — | Comma-separated scopes advertised as `scopes_supported` in the protected-resource metadata, telling clients what to request. Unset advertises nothing (unchanged behaviour). Set it for IdPs that reject an authorization request with no `scope` parameter (Microsoft Entra: `AADSTS900144`) |
